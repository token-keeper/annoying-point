...
`/add <한마디>`를 훅이 가로채 `$AP_HOME/inbox/`에 md로 즉시 저장하고 block한 뒤, 백그라운드 포크가 5섹션 context를 붙인다. `/review`는 현재 세션 AI가 대화형으로 진행한다. 런타임은 bash + jq, 대상은 Claude Code · Codex CLI 0.154.0 · Cursor Agent CLI 2026.09.10.
...
### 커밋 1 — `feat(hook): /add 캡처 훅 ap-capture.sh + hooks.json (Claude)`
...
- 검증:
  1. 단위 — `echo '{"prompt":"/add 표 너무 김","session_id":"s1","transcript_path":"/t.jsonl","cwd":"'"$PWD"'"}' | AP_HOME=$TMP bash scripts/ap-capture.sh` → stdout이 `{"decision":"block","reason":"📌 ap 저장됨 #… (정상 — 훅이 가로챔, 답변 없음) · 30초 뒤 상황 요약 자동 첨부"}`, `$TMP/inbox/`에 md 1개, frontmatter 10개 키 존재(`context: pending` 포함), `stat -f %Lp` = 600
  2. 단위 — `/add +좋음` → `kind: good`, 본문에 `+` 없음 · `/add` 단독 → 사용법 block, 파일 0개 · `$review x` → 출력 없음 exit 0 · `hello` → 출력 없음 exit 0 · `$add x` → 캡처
  ...
  5. 라이브 — `claude --plugin-dir /Users/mini/Github/ai-tools/annoying-point` 새 세션에서 `/add 테스트` → 화면에 block 한 줄, AI 응답 없음, 세션 transcript의 assistant 턴 수 변화 0 (**검증 항목 1**)
...
- 검증:
  ...
  5. 라이브 — `claude --plugin-dir` 세션에서 `/add 테스트` → 훅 즉시 반환 + 60초 내 md 완성. 훅 프로세스가 포크를 기다리지 않음(`ps`로 훅 종료 확인)
...
- 검증:
  ...
  3. 라이브 Codex — `/add 테스트` 입력 → md 생성 + block 표시 (**검증 항목 2·3**). 포크 → `## context` 완성 (**검증 항목 4**)
  4. 라이브 Cursor — `/add 테스트` 입력 → md 생성 + 프롬프트 중단 (**검증 항목 2**). 포크 → `## context` 완성, `conversation_id`로 `--resume` 성공 (**검증 항목 5**)
...
### 커밋 5 — `feat(skill): review SKILL.md`

- 산출: `skills/review/SKILL.md`(TECH_SPEC §7.2 골격을 채운 전문 — 읽기·묶기·진단·제안·승인·반영·보고 7단계, pending 5분 규칙 · cwd 경로 확인 그룹핑 · 적용 실패 시 보류 · `mv -n` 이동, 표준 하네스 경로 목록, CLI 도구명 없음)
...
- 검증:
  ...
  2. 라이브 Claude — `/review` → 그룹 3개(빈 target은 추정 태그로 편입) → 그룹마다 진단·diff 1개 → 1/2/3 질문이 **한 번에 하나씩** → 1 선택 시 파일 수정 + `processed/2026-09/`로 이동 + `resolution:` 추가 → 3 선택 시 inbox 잔류 → 마지막에 바뀐 파일 1줄. 커밋·push 시도 0회
  3. 라이브 Codex(`$review`)·Cursor — 스킬이 로딩되고 같은 절차로 진행되는지 1회씩
  4. 정적 — `grep -n 'AskUserQuestion\|Task(' skills/review/SKILL.md` → 0건 (CLI 특정 도구명 금지)
...
| 9 | `/review` 대화형 절차 | 그룹·진단·diff·1/2/3 하나씩·processed 이동·resolution | 5 |
...
| 마일스톤 | 포함 커밋 | 종료 조건 |
|---|---|---|
| **M1 — Claude E2E** | 커밋 1~2 | Claude에서 `/add` → 즉시 block + 60초 내 context 완성. 체크리스트 1~5 통과 |
| **M2 — 3 CLI + 알림** | 커밋 3~4 | Codex·Cursor에서 같은 동작, install.sh 멱등, SessionStart 1줄. 체크리스트 6~8 통과 |
| **M3 — 리뷰·문서** | 커밋 5~6 | `/review` 대화형 절차 동작, README 재현. 체크리스트 9~10 통과 |

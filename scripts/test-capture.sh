...
echo "== 검증 1: 기본 캡처"
H="$TMP/c1"; run "$H" "$(j '/add 표 너무 김')"; MD="$(md1 "$H")"
...
H="$TMP/c2"; run "$H" "$(j '/add +좋음')"; MD="$(md1 "$H")"
assert "2 +좋음 kind: good" grep -qx 'kind: good' "$MD"
assert "2 +좋음 본문에 + 없음" [ "$(sed -n 13p "$MD")" = 좋음 ]
H="$TMP/c2b"
run "$H" "$(j '/add')";        assert_has "2 /add 단독 사용법" "$(reason "$OUT")" "ap 사용법"
run "$H" "$(j '/add   ')";     assert_has "2 /add 공백만 사용법" "$(reason "$OUT")" "ap 사용법"
run "$H" "$(j '/add +')";      assert_has "2 /add + 사용법" "$(reason "$OUT")" "ap 사용법"
run "$H" "$(j '/add +  ')";    assert_has "2 /add +공백 사용법" "$(reason "$OUT")" "ap 사용법"
assert "2 사용법 케이스 파일 0" [ "$(nfiles "$H")" = 0 ]
run "$H" "$(j '/add-review x')"; assert "2 /add-review 출력 없음" [ -z "$OUT" ]; assert "2 /add-review exit 0" [ "$RC" = 0 ]
run "$H" "$(j 'hello')";        assert "2 hello 출력 없음" [ -z "$OUT" ];      assert "2 hello exit 0" [ "$RC" = 0 ]
run "$H" "$(j '/addx')";         assert "2 /addx 출력 없음" [ -z "$OUT" ]
# v0.2.0 회귀 — 구 이름 $ap·/annoying-point:ap 과 리뷰 호출 $review 는 캡처하지 않는다 (호환 없음)
run "$H" "$(j '$ap 점점')";     assert "2 구 \$ap 출력 없음·exit 0" [ -z "$OUT" -a "$RC" = 0 ]
run "$H" "$(j '/annoying-point:ap 점점')"; assert "2 구 /annoying-point:ap 출력 없음·exit 0" [ -z "$OUT" -a "$RC" = 0 ]
run "$H" "$(j '$review 이번주')"; assert "2 \$review 평문 출력 없음·exit 0" [ -z "$OUT" -a "$RC" = 0 ]
assert "2 통과 케이스 파일 0" [ "$(nfiles "$H")" = 0 ]
run "$H" "$(j '$add x')"; MD="$(md1 "$H")"
assert "2 \$add x 캡처" [ "$(nfiles "$H")" = 1 ]
assert "2 \$add x 본문" [ "$(sed -n 13p "$MD")" = x ]
H="$TMP/c2c"; run "$H" "$(j $'/add 첫줄\n둘째줄')"; MD="$(md1 "$H")"
...
H="$TMP/c2e"; run "$H" "$(jc '/add' 'x' '/add x')"
assert "2b command_name=/add 캡처" [ "$(nfiles "$H")" = 1 ]
assert "2b command_name=/add 본문" [ "$(sed -n 13p "$(md1 "$H")")" = x ]
H="$TMP/c2f"; run "$H" "$(jc 'other:cmd' 'x' '/other:cmd x')"
assert "2b 타 커맨드 → 출력 없음" [ -z "$OUT" ]; assert "2b 타 커맨드 → exit 0" [ "$RC" = 0 ]
run "$H" "$(jc 'other:cmd' '' '/add x')"
assert "2b 타 커맨드는 prompt 가 /add 여도 통과" [ -z "$OUT" ]
assert "2b 타 커맨드 파일 0" [ "$(nfiles "$H")" = 0 ]
H="$TMP/c2g"; run "$H" "$(jc 'annoying-point:add' '' '/annoying-point:add 표 김')"
assert "2b command_args 없음 → prompt 첫 토큰 제거" [ "$(sed -n 13p "$(md1 "$H")")" = "표 김" ]
H="$TMP/c2h"; run "$H" "$(jc 'annoying-point:add' '' '표 김')"
assert "2b command_args 없음·prompt 인자만 → 그대로" [ "$(sed -n 13p "$(md1 "$H")")" = "표 김" ]
H="$TMP/c2i"; run "$H" "$(jc 'annoying-point:add' '' '/annoying-point:add')"
assert_has "2b 커맨드 인자 없음 → 사용법" "$(reason "$OUT")" "ap 사용법"
assert "2b 커맨드 인자 없음 → 파일 0" [ "$(nfiles "$H")" = 0 ]
H="$TMP/c2j"; run "$H" "$(jc 'annoying-point:add' '+좋음' '/annoying-point:add +좋음')"
assert "2b 커맨드 +좋음 → good" grep -qx 'kind: good' "$(md1 "$H")"
H="$TMP/c2k"; run "$H" "$(j '/annoying-point:add 표 김')"; MD="$(md1 "$H")"
assert "2b 플레인 /annoying-point:add 캡처" [ "$(nfiles "$H")" = 1 ]
assert "2b 플레인 /annoying-point:add 본문" [ "$(sed -n 13p "$MD")" = "표 김" ]
run "$H" "$(j '/annoying-point:addx y')"; assert "2b /annoying-point:addx 출력 없음" [ -z "$OUT" ]
run "$H" "$(j '/annoying-point:add-review y')"; assert "2b /annoying-point:add-review 출력 없음" [ -z "$OUT" ]
...
H="$TMP/c2l"; runc "$H" "$(jcu '/add 커서 테스트')"; MD="$(md1 "$H")"
...
runc "$H" "$(jcu '/add')"; assert "2c 사용법도 cursor 형식" [ "$(printf '%s' "$OUT" | jq -r '.continue, (.user_message|startswith("📌 ap 사용법"))' | tr '\n' ' ')" = "false true " ]
OUT="$(printf '%s' "$(jcu '/add 홈 밖')" | AP_HOME=/tmp/x /bin/bash "$CAP" --agent cursor 2>&1)"
...
H="$TMP/c3b"; run "$H" '{"prompt":"/add t","session_id":"s1","transcript_path":null,"cwd":"'"$ROOT_DIR"'"}'
assert "3 transcript null → -" grep -qx 'transcript: -' "$(md1 "$H")"
NG="$(mktemp -d)"; H="$TMP/c3c"; run "$H" "$(j '/add t' "$NG")"; MD="$(md1 "$H")"
...
OUT="$(printf '%s' "$(j '/add x')" | AP_HOME="$TMP/c4" PATH=/bin /bin/bash "$CAP" 2>&1)"; RC=$?
...
mkfake_fork; H="$TMP/c4s"; run "$H" '{"prompt":"/add s","session_id":"","cwd":"'"$ROOT_DIR"'"}'; MD="$(md1 "$H")"
...
H="$TMP/c4b"; run "$H" "$(jq -cn --arg c "$ROOT_DIR" '{prompt:"/add 메타",session_id:"s\n1",transcript_path:"/t\r\n2",cwd:$c}')"; MD="$(md1 "$H")"
...
run "$HOME/x/../../tmp" "$(j '/add 점점')"; assert_has "4b \$HOME/x/../../tmp → 저장 실패" "$(reason "$OUT")" "저장 실패"
assert "4b .. 경로 파일 0" [ ! -e "$HOME/x" -a ! -e /tmp/inbox ]
OUTSIDE="$(mktemp -d)"; ln -s "$OUTSIDE" "$TMP/link-out"; run "$TMP/link-out" "$(j '/add 심링크')"
assert_has "4b 홈 안 심링크 → 홈 밖 → 저장 실패" "$(reason "$OUT")" "저장 실패"
assert "4b 심링크 너머 inbox 파일 0" [ "$(ls "$OUTSIDE/inbox" 2>/dev/null | wc -l | tr -d ' ')" = 0 ]
rm -rf "$OUTSIDE"
mkdir -p "$TMP/real-in"; ln -s "$TMP/real-in" "$TMP/link-in"; run "$TMP/link-in" "$(j '/add 홈안링크')"
...
H="$TMP/c5"; OUT="$(printf '%s' "$(j '/add 배선')" | AP_HOME="$H" AP_FORK_CMD="$FAKE" FAKE_OUT="$FAKE_OUT" /bin/bash "$ROOT_DIR/scripts/ap-capture.sh" 2>&1)"; RC=$?
...
H="$TMP/c5b"; run "$H" "$(j '/add 분리')"; for i in 1 2 3 4 5 6; do [ -s "$H/pgid" ] && break; sleep 0.5; done
...
H="$TMP/c5c"; OUT="$(printf '%s' "$(j '/add 폴백')" | AP_HOME="$H" PATH="$TMP/nobin" /bin/bash "$CAP" 2>&1)"
...
RES="$(python3 - "$CAP" "$TMP/c6" "$(j '/add 표 너무 김')" <<'PY'
